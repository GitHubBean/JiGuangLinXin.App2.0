    ...
    public class MallGoodsTypeController : BaseController
    {
        ...
        public ResultMessageViewModel EditList([FromBody] JObject value)
        {
            ...
            if (itemList.Any())
            {
                ...
                {
                    Core_MallType item = new Core_MallType()
                    {
                    ...
                    if (string.IsNullOrEmpty(item.T_Title))
                    {
                        continue;
        ...
        public ResultMessageViewModel Add([FromBody] JObject value)
        {
            ...
            Core_MallType tp = new Core_MallType()
            {
                T_Rank = obj.rank,
                T_Recom = obj.recom,
                T_State = obj.state,
        ...
        public ResultMessageViewModel Edit([FromBody] JObject value)
        {
            ...
            dynamic obj = value;
            int tid = obj.tid;
            Core_MallType tp = typeCore.LoadEntity(o => o.T_Id == tid);
            if (tp != null)
            {
                tp.T_Rank = obj.rank;
        ...
        public ResultMessageViewModel RemoveType([FromBody] JObject value)
        {
            ...
            {
                new MallGoodsCore().UpdateByExtended(o => new Core_MallGoods() { G_Status = 1 }, o => o.G_TypeId == tid); //批量修改此类型商品状态为下架
                rs.State = 0;
                rs.Msg = "ok";
            }
