...
namespace JiGuangLinXin.App.BusinessCenter.Controllers
{
    public class GroupbuyController : BaseController
    {
        ...
        public ResultMessageViewModel ProList([FromBody] JObject value)
        {
            ...
            if (state == 0)  //团购中
            {
                exp =
                    exp.And(
                        o =>
                            o.GB_AuditingState == (int)AuditingEnum.认证成功 && o.GB_STime < DateTime.Now &&
                            o.GB_ETime > DateTime.Now && o.GB_State == 0);
            }
        ...
        {
            if (stime < DateTime.Now && etime > DateTime.Now && joinCount == peoCount)
            {
