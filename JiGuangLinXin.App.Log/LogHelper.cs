...
namespace JiGuangLinXin.App.Log
{
    ...
    {
        //日志对象的缓存队列
        private static Queue<LogMsg> msgs;
        //日志文件保存的路径
        ...
        private void FileWrite(LogMsg msg)
        {
            try
            {
                ...
                writer.WriteLine("日志时间："+msg.Datetime);
                //writer.WriteLine('\t');
                writer.WriteLine("日志级别："+msg.Type);
                //writer.Write('\t');
                writer.WriteLine("日志内容："+msg.Text);
                writer.WriteLine("******************************");
                writer.Flush();
            }
            catch (Exception e)
        ...
        public void Write(string text, LogLevel type)
        {
        ...
        /// <param name="type">信息类型</param>
        public void Write(Exception e, LogLevel type)
        {
            Write(new LogMsg(e.Message, type));
        }
