        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            try
            {
                ...
                else//校验通过
                {
                    ...
                    #region 暂时屏蔽的验证 目前不做社区认证过滤 2016-05-26 16:34:40
                    ...
                    //    {
                    //        ResultViewModel rs = new ResultViewModel() { State = 1, Msg = "用户帐号已被冻结！", Data = null };
                    //        actionContext.Response = new HttpResponseMessage { Content = new StringContent(JsonSerialize.Instance.ObjectToJson(rs), Encoding.GetEncoding("UTF-8"), "application/json") };
                    //    }
                    //    else if (user.U_AuditingState != (int)AuditingEnum.认证成功)
                    //    {
                    //        ResultViewModel rs = new ResultViewModel() { State = 1, Msg = "您已入驻自己小区，请通过社区认证！", Data = null };
                    //        actionContext.Response = new HttpResponseMessage { Content = new StringContent(JsonSerialize.Instance.ObjectToJson(rs), Encoding.GetEncoding("UTF-8"), "application/json") };
                    //    }
                    //    else
                    //    {
