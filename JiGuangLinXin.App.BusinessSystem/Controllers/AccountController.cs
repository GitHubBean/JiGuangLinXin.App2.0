...
namespace JiGuangLinXin.App.BusinessSystem.Controllers
{
    public class AccountController : ApiController
    {
        ...
        public ResultMessageViewModel Login([FromBody] JObject value)
        {
            ...
            else if (admin.B_AuditingState != (int)AuditingEnum.认证成功)
            {
                rs.Msg = "帐号暂未通过审核" + admin.B_AuditingState;
            }
            else
            {
                ...
                {
                    FormTicketHelper.SetCookie(admin.B_LoginPhone, string.Format("{0},{1}", admin.B_Role, admin.B_Id),
                        rememberMe == 1, "bcenter_admin_");
        ...
        public ResultMessageViewModel ResetPwd([FromBody] JObject ob)
        {
            ...
            if (!enCode.Equals(CacheHelper.GetCacheString("ResetPwdCode" + phone)))  //验证码
            {
                rs.State = 2;
                rs.Msg = "验证码错误";
            }
