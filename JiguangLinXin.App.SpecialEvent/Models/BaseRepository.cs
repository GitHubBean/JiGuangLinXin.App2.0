...
namespace JiguangLinXin.App.SpecialEvent.Models
{
    public partial class BaseRepository<T> where T : class
    {
        ...
        /// <param name="parameter">参数(可选)</param>
        /// <returns></returns>
        public IList<T> ExecuteStoreQuery(string commandText, params Object[] paramete)
        {
            var result = db.Database.SqlQuery<T>(commandText, paramete).ToList<T>();
            return result;
    ...
    public class EFContextFactory
    {
        ...
        public static DbContext GetCurrentDbContext()
        {
            ...
            if (dbcontext == null)
            {
                dbcontext = new SpecialEventEntities();  //创建了一个EF上下文
                //存储指定对象
