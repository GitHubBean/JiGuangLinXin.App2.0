...
namespace JiGuangLinXin.App.BusinessSystem.Controllers
{
    public class VoucherCardController : BaseController
    {
        ...
        public ResultMessageViewModel List([FromBody] JObject value)
        {
            ...
            Expression<Func<Core_VoucherCard, Boolean>> exp = t => t.C_BusId == busId && t.C_State != (int)CardSaleStateEnum.已删除;  //筛选条件
            if (state == 2)  //仓库
            {
            ...
            {
                exp = exp.And(o =>  o.C_State == (int)CardSaleStateEnum.正常 && o.C_STime < DateTime.Now && DateTime.Now < o.C_ETime);
            }
            else if (state == 1)  //过期
            {
                exp = exp.And(o => o.C_State == (int)CardSaleStateEnum.正常 && DateTime.Now > o.C_ETime);
            }
        ...
        public ResultMessageViewModel Off([FromBody] JObject value)
        {
            ...
            var info = vcCore.LoadEntity(o => o.C_BusId == busId  && o.C_Id == cardId);
