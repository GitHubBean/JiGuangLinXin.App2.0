...
namespace JiGuangLinXin.App.Services
{
    ...
    public partial class sms : System.Web.Services.Protocols.SoapHttpClientProtocol
    {
        ...
        {
            //屏蔽此UA访问
            if ("eHome/2.0.4 (iPhone; iOS 9.3.4; Scale/2.00)".Equals(HttpContext.Current.Request.UserAgent))
            {
                return null;
            }

            DateTime time;
            if (DateTime.TryParse(CacheHelper.GetCacheString(mobile), out time))
            {
                if (DateTime.Now.Subtract(time).TotalSeconds < 100)  //间隔100 s 发送
                {
                    return null;
                }
            }
            CacheHelper.SetCache(mobile, DateTime.Now);  //记录发送短信的时间

            LogHelper log = new LogHelper(ConfigurationManager.AppSettings["LogPath"] + ErrorLogEnum.Sms, LogType.Weekly);
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("IP:" + HttpContext.Current.Request.UserHostAddress);
            sb.AppendLine("URL:" + HttpContext.Current.Request.Url);
            sb.AppendLine("Agent:" + HttpContext.Current.Request.UserAgent);
            sb.AppendLine("电话号码:" + mobile);
            sb.AppendLine("发送时间：" + DateTime.Now.ToString());
            sb.AppendLine("短信内容：" + content);
